...
assert.match(workflow, /Get-Item 'apps\/dim-sum-atlas\/dist\/squirrel-windows\/update\.json'/);
assert.match(read('scripts/generate-update-metadata.mjs'), /candidate \? 'update-metadata\.candidate\.json' : 'update\.json'/);
assert.match(read('apps/dim-sum-atlas/src/updater/feed-config.js'), /releases\/latest\/download\/update\.json/);
