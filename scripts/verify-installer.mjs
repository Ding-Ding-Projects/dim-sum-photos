...
const signatureArg=process.argv.find(a=>a.startsWith('--signature-status=')); const signature=signatureArg?.split('=')[1] ?? (process.argv.includes('--signature-status') ? process.argv[process.argv.indexOf('--signature-status')+1] : undefined); if(signature && signature!=='NotSigned') throw new Error(`Unexpected signature status: ${signature}`);
const result={dist,setup:{file:setup,sha256:hash(setup),bytes:fs.statSync(path.join(dist,setup)).size},RELEASES:{file:releases,sha256:hash(releases),references:nupkg},nupkg:{file:nupkg,sha256:hash(nupkg)},verifiedAt:new Date().toISOString()}; if(signature) result.signatureStatus=signature;
